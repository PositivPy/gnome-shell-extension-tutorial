...
this.actor : inherited from PanelMenu.ButtonBox, it is a Shell.GenericContainer() into wich you can add a single St element, like a St.Label(), an St.Icon() or St.Entry(). 
...
### Events 
this.actor is as well a clutter actor :

    this.actor.connect('button-press-event', function(){ Main.notify('Example Notification', 'Hello World !') });

Each UI containers that we will study here have a clutter actor and by default, each clutter actors have these events:
...
We can "connect" each actors to event(s) and than to callback function(s). 
The callback function will be called when the event occurs.

    this.actor.connect("event-name", callback);

### PopupMenuItems
this.menu : PanelMenu.Button adds a PopupDummyMenu to Panel.ButtonBox. Essentialy, a menu constuctor to wich we can add menu items with

    this.menu.addMenuItem(menuItem);

Here are the menu items available :
